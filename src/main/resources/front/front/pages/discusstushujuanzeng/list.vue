			...
			<view v-if="list.length" class="list-box5" :style='{"minHeight":"100vh","padding":"100rpx 24rpx 0","margin":"40rpx 0 0","borderRadius":"0 16rpx 16rpx 0","flex":"1","background":"#fff","width":"calc(100% - 220rpx)","height":"auto"}'>
				...
				<view v-if="index%2==0" class="list-item" :style='{"padding":"20rpx","margin":"0 0 20rpx","borderRadius":"16rpx","flexWrap":"wrap","background":"#f4f4f4","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"auto"}'>
					...
					<view :style='{"width":"100%","padding":"8rpx 0px","margin":"20rpx 0 0","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('discusstushujuanzeng','修改')) || (!userid && isAuthFront('discusstushujuanzeng','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('discusstushujuanzeng','删除')) || (!userid && isAuthFront('discusstushujuanzeng','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
					...
					<!-- #ifndef MP-WEIXIN -->
					<view :style='{"width":"100%","padding":"8rpx 0px","margin":"20rpx 0 0","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('discusstushujuanzeng','修改')) || (!userid && isAuthFront('discusstushujuanzeng','修改'))" @tap.stop.proevent="onUpdateTap(product)">
							<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-edit"></text>
							<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>修改</text>
						</view>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('discusstushujuanzeng','删除')) || (!userid && isAuthFront('discusstushujuanzeng','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
							<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-delete"></text>
							<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>删除</text>
						</view>
					</view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`discusstushujuanzeng`, params);
                } else {
                    res = await this.$api.list(`discusstushujuanzeng`, params);
                }
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('discusstushujuanzeng', JSON.stringify([id]));
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`discusstushujuanzeng`, searchForm);
                } else {
                    res = await this.$api.list(`discusstushujuanzeng`, searchForm);
                }
				// 如果是第一页数据置空
