...
<view class="content">
	<view :style='{"minHeight":"100vh","width":"100%","padding":"24rpx","position":"relative","background":"#f7f7f7","height":"auto"}'>
		<form :style='{"width":"100%","padding":"24rpx","borderRadius":"16rpx","background":"#fff","display":"block","height":"auto"}' class="app-update-pv">
			...
			<view :style='{"padding":"12rpx 0","margin":"0 0 24rpx 0","borderColor":"#ccc","alignItems":"center","borderWidth":"0 0 2rpx 0","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				<view :style='{"width":"160rpx","padding":"0 20rpx 0 0","lineHeight":"80rpx","fontSize":"28rpx","color":"#333","textAlign":"right"}' class="title">捐赠时间</view>
				<input :disabled="ro.juanzengshijian" :style='{"border":"0","padding":"0px 24rpx","margin":"0px","color":"rgb(0, 0, 0)","borderRadius":"8rpx","flex":"1","background":"rgba(255, 255, 255, 0)","fontSize":"28rpx","height":"80rpx"}' v-model="ruleForm.juanzengshijian" placeholder="捐赠时间" @tap="toggleTab('juanzengshijian')"></input>
			</view>
		...
		<w-picker  mode="dateTime" step="1" :current="false" :hasSecond="false" @confirm="juanzengshijianConfirm" ref="juanzengshijian" themeColor="#333333"></w-picker>
...
<script>
	...
	export default {
		...
		async onLoad(options) {
            this.ruleForm.juanzengshijian =  this.$utils.getCurDateTime();
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`tushujuanzeng`, this.ruleForm.id);
		...
		methods: {
			...
			// 日长控件选择日期时间
			juanzengshijianConfirm(val) {
				console.log(val)
				this.ruleForm.juanzengshijian = val.result;
				this.$forceUpdate();
			},
			// 日长控件选择日期时间
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`tushujuanzeng`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`tushujuanzeng`, this.ruleForm);
						}else{
							await this.$api.add(`tushujuanzeng`, this.ruleForm);
						}
