	...
	<view :style='{"minHeight":"100vh","width":"100%","padding":"24rpx","position":"relative","background":"#f7f7f7","height":"auto"}'>
		<form :style='{"width":"100%","padding":"24rpx","borderRadius":"16rpx","background":"#fff","display":"block","height":"auto"}' class="app-update-pv">
			...
			<view :style='{"padding":"12rpx 0","margin":"0 0 24rpx 0","borderColor":"#ccc","borderWidth":"0 0 0px 0","width":"100%","borderStyle":"solid","height":"auto"}' class="">
                ...
                <xia-editor ref="editor" :style='{"border":"2rpx solid #efefef","minHeight":"360rpx","width":"100%","padding":"20rpx","height":"auto"}' v-model="ruleForm.content" placeholder="评论内容" @editorChange="contentChange"></xia-editor>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussshenqingjiaohuan`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussshenqingjiaohuan`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussshenqingjiaohuan`, this.ruleForm);
						}else{
							await this.$api.add(`discussshenqingjiaohuan`, this.ruleForm);
						}
						uni.setStorageSync('discussshenqingjiaohuanCleanType',true);
